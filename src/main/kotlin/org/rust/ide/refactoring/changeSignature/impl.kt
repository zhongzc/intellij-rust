...
fun processFunctionUsage(config: RsChangeFunctionSignatureConfig, usage: RsFunctionUsage) {
    ...
    if (usage.isCallUsage && config.parametersChanged()) {
...
private fun changeArguments(
    ...
    val parameters = when (usage) {
        is RsFunctionUsage.FunctionCall -> usage.call.valueArgumentList
        is RsFunctionUsage.MethodCall -> usage.call.valueArgumentList
        else -> error("unreachable")
    }
    val parametersCopy = parameters.copy() as RsValueArgumentList
    val parametersList = parametersCopy.exprList
    val isUFCS = isMethod && usage is RsFunctionUsage.FunctionCall
    fixParametersOrder(
        factory,
        parameters,
        parametersCopy,
        if (isUFCS) parametersList.first() else null,
        if (isUFCS) parametersList.drop(1) else parametersList,
        config
...
private fun changeParameters(factory: RsPsiFactory, function: RsFunction, config: RsChangeFunctionSignatureConfig) {
    ...
    if (!config.parametersChanged()) return
...
private fun fixParametersOrder(
    ...
    val isSingleParameter = descriptors.size == 1 && descriptors.size < config.originalParameters.size
    ...
    if (isSingleParameter) {
        ...
        if (hasTrailingComma) parameters.addBefore(factory.createComma(), rparen)
